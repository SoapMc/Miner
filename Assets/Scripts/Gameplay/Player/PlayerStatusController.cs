    ...
    public class PlayerStatusController : MonoBehaviour
    {
        ...
        private void CheckForHit()
        {
            ...
            if (sqrSpeedChange > _resistanceToHit && _gridPosition.Value.y < 0)
            {
                ...
                HullReferencePart hull = (HullReferencePart)_equipment.GetEquippedPart(EPartType.Hull);
                if (hull != null)
                {
                    if (damage >= hull.MaxHull * hull.PermaDamageThreshold)
                    {
                    ...
                    _playerDamaged.Raise(new DamagePlayerEA(damage, _damageTypeFromHit));
                }
