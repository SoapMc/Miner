    ...
    public class ServiceWindow : Window
    {
        ...
        public void Repair()
        {
            ...
            if(repairCost <= _playerMoney.Value)
            {
                ...
                _playerRepaired.Raise(new RepairPlayerEA(_playerMaxHull.Value - _playerHull.Value));
            }
            else
            {
                _updatePlayerData.Raise(new UpdatePlayerDataEA() { MoneyChange = -_playerMoney });
                _playerRepaired.Raise(new RepairPlayerEA(Mathf.CeilToInt(_playerMoney.Value / (float)_repairCostPerPoint)));
            }
            _repairSound.Play();
            CalculateRepairCost();
        }
        ...
        public int BuyItem(UsableItem item)
        {
            if(_playerMoney >= item.Cost)
            {
                ...
                cui.AddedUsableItems.Add(new UsableItemTable.Element() { Item = item, Amount = 1 });
